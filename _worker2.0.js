...
export default {
  async fetch(r, e, c) {
    try {
      ...
      if ("/sub" === u.pathname) {
        if (u.searchParams.get(atob('dXVpZA==')) !== UUID) return new Response("Invalid", { status: 403 });
...
async function hSub(r,c,u,UA,h){
  ...
  const iS=[_d('U2luZy1ib3g='),_d('SGlkZGlmeQ==')].includes(cn),iC=[_d('Q2xhc2g='),_d('TWlob21v'),_d('RmxDbGFzaA==')].includes(cn);
  if(iS&&!flg){const t=u.searchParams.get("proxyip");let n=`https://${h}/${SUB_PWD}?flag=true`;t&&(n+=`&proxyip=${encodeURIComponent(t)}`);const bU=`${SUBAPI}/sub?target=${atob('c2luZ2JveA==')}&url=${encodeURIComponent(n)}`,suf="&emoji=true&list=false&sort=false&fdn=false&scv=false&_t="+now;let o=await fetch(bU+`&config=${encodeURIComponent(SBV11)}`+suf);if(!o.ok)o=await fetch(bU+`&config=${encodeURIComponent(SBV12)}`+suf);if(!o.ok)return new Response("Err",{status:500});const hd=new Headers(o.headers);hd.set("Cache-Control","no-store");return new Response(o.body,{status:200,headers:hd});}
  if(iC&&!flg){const t=u.searchParams.get("proxyip");let n=`https://${h}/${SUB_PWD}?flag=true`;t&&(n+=`&proxyip=${encodeURIComponent(t)}`);const a=`${SUBAPI}/sub?target=${atob('Y2xhc2g=')}&url=${encodeURIComponent(n)}&config=${encodeURIComponent(SUBINI)}&emoji=true&list=false&tfo=false&scv=false&fdn=false&sort=false&_t=${now}`,s=await fetch(a);if(!s.ok)return new Response("Err",{status:500});const hd=new Headers(s.headers);hd.set("Cache-Control","no-store");return new Response(s.body,{status:200,headers:hd});}
  let up=SUB.trim().replace(/^https?:\/\//,"").replace(/\/$/,"")||h,pip=u.searchParams.get("proxyip");if(!pip&&PIP)pip=PIP;
  let tp=(pip&&pip.trim())?`/proxyip=${pip.trim()}`:"/";
  const p=new URLSearchParams();p.append(atob('dXVpZA=='),UUID);p.append("host",up);p.append("sni",up);p.append("path",tp);p.append("type","ws");p.append(atob('ZW5jcnlwdGlvbg=='),"none");p.append(atob('c2VjdXJpdHk='),atob('dGxz'));p.append(atob('YWxwbg=='),"h3");p.append("fp","random");p.append(atob('YWxsb3dJbnNlY3VyZQ=='),"1");
